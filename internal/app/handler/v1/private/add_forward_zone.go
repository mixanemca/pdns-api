...
func (s *AddForwardZoneHandler) AddForwardZonesInternal(w http.ResponseWriter, r *http.Request) {
	...
	for _, inputFZ := range input {
		if forwardzone.ForwardZoneIsExist(fzs, infrastructure.Canonicalize("+"+inputFZ.Name)) {
	...
	for _, inputFZ := range input {
		s.logger.WithFields(logrus.Fields{
			...
			"forward-zone": infrastructure.DeCanonicalize(inputFZ.Name),
		}).Infof("Forward zone %s added", infrastructure.DeCanonicalize(inputFZ.Name))
	}
	w.WriteHeader(http.StatusCreated)
	s.stats.CountError(s.config.Environment, infrastructure.GetHostname(), r.URL.Path, http.StatusCreated)
}
